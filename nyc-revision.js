...
(() => {
  ...
  const metCard=[...document.querySelectorAll('#date-panel-2026-10-11 .time-row')].find(el=>el.querySelector('strong')?.textContent.includes('The Met Fifth Avenue'))?.querySelector('.time-card');
  metCard?.insertAdjacentHTML('beforeend',`<div class="actions">${link('https://engage.metmuseum.org/admission','The Met（大都会艺术博物馆）官网购票')}${link('https://www.metmuseum.org/plan-your-visit','参观与入场规则')}</div>`);
  ...
  day('2026-10-13','10月13日 · Top of the Rock（洛克菲勒观景台）、Fifth Avenue（第五大道）、The High Line（高线公园）与 Chelsea Market（切尔西市场）','上午登顶与购物 → 高线和小吃 → 16:00取行李 → 17:00去机场酒店',[
    ...
    ['17:00—19:30','Manhattan（曼哈顿）→ JFK（肯尼迪机场）附近酒店','工作日晚高峰给车程和办理入住留余量。机场前夜酒店尚未预订；预订后导航至酒店完整地址，不要只导航到 JFK（肯尼迪机场）。入住时确认凌晨班车是否需预约、能否直达 Terminal 8（8号航站楼）；若只到 Federal Circle（联邦环路站），还须换 AirTrain（机场捷运）。']
  ...
  document.querySelector('#date-panel-2026-10-13 .ops').insertAdjacentHTML('beforeend',`<details class="city-reference"><summary>博物馆替换项 · 官方购票入口</summary><p>以下都不是本日主线；若决定入馆，须替换 The High Line（高线公园）与购物，不叠加。去 Whitney Museum（惠特尼美国艺术博物馆）也须重新核算17:00去机场的时间。</p><div class="actions">${link('https://visit.moma.org/select','MoMA（纽约现代艺术博物馆）官网购票')}${link('https://www.frick.org/tickets','The Frick Collection（弗里克收藏馆）官网购票')}${link('https://whitney.org/visit','Whitney Museum（惠特尼美国艺术博物馆）官网购票')}</div></details>`);
  document.querySelector('#date-panel-2026-10-13 .ops').insertAdjacentHTML('beforeend','<p><b>机场前夜酒店待订：</b>可比较 Courtyard by Marriott New York JFK Airport（纽约肯尼迪机场万怡酒店）、Hampton Inn NY-JFK（纽约肯尼迪机场欢朋酒店）与 TWA Hotel（环球航空酒店）。万怡预订页写明24小时、每半小时离店班车可送到指定航站楼；订前直接向酒店核实02:30左右班次及 Terminal 8（8号航站楼）停靠。次日06:30的 AA475（美国航空）以03:15—03:30到T8为目标。<a target="_blank" rel="noopener" href="https://www.jfkairport.com/explore-jfk/hotels">JFK（肯尼迪机场）酒店接驳说明</a></p>');
  ...
  if(body){
    ...
    body.innerHTML=body.innerHTML.replace('10:00—11:45大都会艺术博物馆','10:00—11:30大都会艺术博物馆').replace('石街午餐后付费渡轮至丹波区，体力允许走布鲁克林大桥','利奥贝果外带后换付费渡轮至丹波区；14:30前到入口才过布鲁克林大桥').replace('观景台未购票。</p>','观景台未购票，机场酒店待订。</p>');
  }
})();
