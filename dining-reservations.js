/* Reservation status and reminders; confirmed entries are based on user-provided confirmations. */
(() => {
  const rows=[['2026-10-11','Casa Mono（卡萨莫诺）','已确认','12:45 · 两人 · 室内普通座位','已收到 OpenTable（订位平台）确认。两人须同时到场；建议12:30抵达。迟到宽限10分钟；少于1天取消或未到店，每人收费50美元。午餐座位保留1小时45分钟。','https://www.opentable.com/r/casa-mono-new-york'],
    ...
    ['2026-10-03','Kali Steak（卡利牛排餐厅）','已确认','19:45 · 两人 · 室内普通座位','OpenTable（订位平台）已确认。天文台18:34日落后去官方网约车区；若叫车预估到店晚于19:45，及时致电餐厅。','https://www.opentable.com/kali'],
  ...
  const overview=document.createElement('details');overview.className='city-reference';overview.id='restaurant-reservations';overview.innerHTML='<summary>餐厅订位清单与入口</summary><div class="reference-body"><p>Kali Steak（卡利牛排餐厅）10月3日19:45与 Casa Mono（卡萨莫诺）10月11日12:45的两人室内座位均已确认；其余餐厅按各自状态查看。未选定的备选餐厅无需占位，改动时核对取消及未到店规则。</p>'+rows.map(r=>'<h4>'+r[0].slice(5).replace('-','月')+'日</h4>'+render(r)).join('')+'</div>';document.getElementById('book').append(overview);
})();
